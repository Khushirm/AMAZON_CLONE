...
const CartPayment = () => {
  ...
  useEffect(() => {
    setTotalAmount(
      productData?.reduce(
        (amt: number, item: StoreProduct) => amt + item.price * item.quantity,
        0
      ) || 0
    );
  }, [productData]);
  ...
  const handleCheckout = async () => {
    ...
    const response = await fetch("/api/checkout", {
      ...
      body: JSON.stringify({ items: productData, email: session?.user?.email, customerName:userInfo?.name || "Guest", customerAddress:userInfo?.address || "Default Address",
       }),
    });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        ...
        <p className="text-sm text-red-500">
          FREE SHIPPING ||CHECKOUT|| See Details...
        </p>
      </div>
